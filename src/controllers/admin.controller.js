...
const { clean, jsonError, parsePagination, serializeRequest } = require("../utils/response");
...
exports.getUsers = async (req, res) => {
  ...
  const selectedStatus = ["active", "blocked"].includes(req.query.status) ? req.query.status : "all";
...
exports.updateUser = async (req, res) => {
  ...
  const update = {};
  if (["active", "blocked"].includes(req.body?.status)) update.status = req.body.status;
  if (["donor", "volunteer", "admin"].includes(req.body?.role)) update.role = req.body.role;
  if (!Object.keys(update).length) return jsonError(res, 400, "Invalid update.");
  ...
  const query = ObjectId.isValid(req.params.id)
    ...
    : { id: req.params.id };
  const result = await users().updateOne(query, { $set: { ...update, updatedAt: new Date() } });
...
exports.getRequests = async (req, res) => {
  const user = req.user;
  if (!["admin", "volunteer"].includes(user.role)) return jsonError(res, 403, "Forbidden.");
  const selectedStatus = requestStatuses.has(req.query.status) ? req.query.status : "all";
  const { page, pageSize } = parsePagination(req.query, 10, 20);
...
exports.updateRequestStatus = async (req, res) => {
  if (!["admin", "volunteer"].includes(req.user.role)) return jsonError(res, 403, "Forbidden.");
  if (!ObjectId.isValid(req.params.id)) return jsonError(res, 400, "Invalid update.");
  const nextStatus = clean(req.body?.status);
  if (!requestStatuses.has(nextStatus)) return jsonError(res, 400, "Invalid update.");
...
exports.getStats = async (req, res) => {
  ...
  const [totalUsers, totalRequests, fundsAgg, statusGroups] = await Promise.all([
    ...
    funds().aggregate([{ $group: { _id: null, total: { $sum: "$amount" } } }]).toArray(),
    donationRequests().aggregate([{ $group: { _id: "$donationStatus", count: { $sum: 1 } } }]).toArray(),
  ]);
  ...
  return res.json({
    ...
    statusCounts: Object.fromEntries(statusGroups.map((item) => [item._id, item.count])),
  });
};
